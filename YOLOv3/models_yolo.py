...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=256):
        ...
        self.criterion_L1_obj = torch.nn.L1Loss()
        ...
        if x1 is not None:
            for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
                if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                    ...
                    x1 = torch.cat([layer_outputs_x1[int(layer_i)] for layer_i in module_def["layers"].split(",")], 1)
                elif module_def["type"] == "shortcut":
                    layer_i = int(module_def["from"])
                    x1 = layer_outputs_x1[-1] + layer_outputs_x1[layer_i]
                elif module_def["type"] == "yolo":
                    temp += 1
